...
<div class="mb-6">
    ...
    <p class="text-gray-500">{{ $session->auditee_name }} — {{ $session->dealer->name ?? '' }}</p>
</div>

<form method="POST" action="{{ route('dealer.genba.submit', $session) }}">
    @csrf
    <div class="space-y-4">
        @foreach($questions as $index => $question)
        <div class="bg-white rounded-xl shadow p-5">
            <div class="mb-3">
                <span class="text-xs text-gray-400 font-medium">No. {{ $index + 1 }}</span>
    ...
    <div class="mt-6 flex gap-3">
        <button type="submit"
            class="flex-1 bg-red-600 text-white py-3 rounded-lg hover:bg-red-700 font-medium text-lg">
            Submit Checklist ✓
        </button>
    </div>
</form>
